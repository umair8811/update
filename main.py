...
#             conn = sqlite3.connect('event_management.db', timeout=10)  
#             cursor = conn.cursor()
#             # Exclude profiles with profile_id 7
#             res = cursor.execute("SELECT * FROM Profile WHERE profile_type_id != 7")
#             res = cursor.fetchall()
#             if not res:
#                 error_list.append("No profiles found in the database.")
#                 return profile_dict_list  # Return empty list if no profiles are found
...
#             conn = sqlite3.connect('event_management.db', timeout=10) 
#             cursor = conn.cursor()
#             # Exclude users with user_type_id = 3 and profile_type_id = 7
#             res = cursor.execute(""" 
#                 SELECT * FROM Users 
#                 WHERE NOT (user_type_id = 3 AND profile_type_id = 7)
#             """)
#             res = cursor.fetchall()
            
#             if not res:
#                 error_list.append("No users found in the database.")
#                 return profile_dict_list  # Return empty list if no users are found
            
#             keys = ['user_id', 'first_name', 'last_name', 'business_name', 'email', 'active_status', 
...
# def Create_Package(Packages:Packages):
   
#     conn = sqlite3.connect('event_management.db', timeout=10) 
#     cursor = conn.cursor()

#     cursor.execute("""INSERT INTO `Packages` (`package_name`,`package_price`,`price`,`user_id`) VALUES (?,?,?,?) """,
#     (Packages.package_name,Packages.package_price,Packages.price,Packages.user_id))    

#     conn.commit()
#     res = cursor.execute(""" Select * from `Packages` """)
#     res =cursor.fetchall();
#     keys = ['package_id','package_name','package_price','price','user_id']
#     Packages_dict_list = [dict(zip(keys, item)) for item in res]
#     conn.commit()
#     conn.close()
...
#     error_list = []
#     executor = ThreadPoolExecutor()
#     def fetch_Package_Details_data():
#         try:
#             conn = sqlite3.connect('event_management.db', timeout=10) 
#             cursor = conn.cursor()
#             res = cursor.execute(""" Select * from `Package_Details` """)
...
#             conn.close()
#             return profile_dict_list
#     future = executor.submit(fetch_Package_Details_data)
#     Package_Details_dict_list = future.result()

#     # Check if there were any errors
#     if error_list:
#         return {"errors": error_list}
  
#     return {"Package_Details ":Package_Details_dict_list}
...
# def Create_Book_Event(Events:Events):
   
#     conn = sqlite3.connect('event_management.db', timeout=10) 
#     cursor = conn.cursor()

#     cursor.execute("""INSERT INTO `Events` (`event_name`,`number_of_guests`,`package_id`,`start_date`,`end_date`,`user_id`) VALUES (?,?,?,?,?,?) """,
#     (Events.event_name,Events.number_of_guests,Events.package_id,Events.start_date,Events.end_date,Events.user_id))    

#     conn.commit()
#     res = cursor.execute(""" Select * from `Events` """)
#     res =cursor.fetchall();
#     keys = ['event_id','event_name','number_of_guests','package_id','start_date','end_date','user_id']
#     Events_dict_list = [dict(zip(keys, item)) for item in res]
#     conn.commit()
#     conn.close()
...
#                        e.user_id, e.payment_status, p.profile_id, p.company_name, pt.profile_type
#                 FROM Events e
#                 JOIN Profile p ON e.user_id = p.user_id
...
#Create Payments
# @app.post("/Create_Payments",status_code=status.HTTP_201_CREATED)
# def Create_Book_Event(Payments:Payments):
   
#     conn = sqlite3.connect('event_management.db', timeout=10) 
#     cursor = conn.cursor()

#     cursor.execute("""INSERT INTO `Payments` (`payment_amount`,`payment_type`,`event_id`,`package_id`,`user_id`) VALUES (?,?,?,?,?) """,
#     (Payments.payment_amount,Payments.payment_type,Payments.event_id,Payments.package_id,Payments.user_id))    
...
#     # Query to join all related tables and get comprehensive information
#     cursor.execute('''
#         SELECT u.user_id, u.first_name, u.last_name, u.business_name, u.email, 
#                u.active_status, u.location, u.contact, 
#                ut.user_type, pt.profile_type,
#                p.company_name, p.contact_detail, p.experience, p.thumbnail_image,
#                pk.package_name, pk.package_price, pkd.package_header_image, pkd.package_image,
#                e.event_name, e.number_of_guests, e.start_date, e.end_date,
#                pay.payment_amount, pay.payment_type, pay.status,
#                mb.event_type, mb.bid_amount, mb.currency, mb.remarks
#         FROM Users u
#         LEFT JOIN User_Type ut ON u.user_type_id = ut.user_type_id
#         LEFT JOIN Profile_Type pt ON u.profile_type_id = pt.profile_type_id
#         LEFT JOIN Profile p ON u.profile_type_id = p.profile_type_id
#         LEFT JOIN Packages pk ON u.user_id = pk.user_id
#         LEFT JOIN Package_Details pkd ON pk.package_id = pkd.package_id
#         LEFT JOIN Events e ON u.user_id = e.user_id
#         LEFT JOIN Payments pay ON u.user_id = pay.user_id
#         LEFT JOIN Mark_bidding mb ON u.user_id = mb.user_id
#     ''')
...
#     # Define keys to structure each user result
#     keys = [
#         'user_id', 'first_name', 'last_name', 'business_name', 'email', 
#         'active_status', 'location', 'contact', 'user_type', 'profile_type',
#         'company_name', 'contact_detail', 'experience', 'thumbnail_image',
#         'package_name', 'package_price', 'package_header_image', 'package_image',
#         'event_name', 'number_of_guests', 'start_date', 'end_date',
#         'payment_amount', 'payment_type', 'status',
#         'event_type', 'bid_amount', 'currency', 'remarks'
#     ]
    
#     # Convert each user result to a dictionary
#     all_users = [dict(zip(keys, user)) for user in users_info]
#     return all_users
...
#     # Query to join all related tables and get information for a specific user_id
#     cursor.execute('''
#         SELECT u.user_id, u.first_name, u.last_name, u.business_name, u.email, 
#                u.active_status, u.location, u.contact, 
#                ut.user_type, pt.profile_type,
...
#     # If no data is found for the given user_id, return None
#     if row is None:
#         return None

#     # Define the keys for the user information
#     keys = ["user_id", "first_name", "last_name", "business_name", "email", "active_status", "location", "contact", 
#             "user_type", "profile_type", "company_name", "contact_detail", "experience", "thumbnail_image",
#             "package_name", "package_price", "package_header_image", "package_image",
#             "event_name", "number_of_guests", "start_date", "end_date",
...
#r @app.get("/user_profiles_information", status_code=status.HTTP_200_OK)
# def get_users():
...
#     # Fetch packages and their details for the specified user_id
#     cursor.execute('''
#         SELECT Users.user_id, Users.first_name, Users.last_name, Packages.package_id,
#                Packages.package_name, Packages.package_price, Package_Details.package_dtl_id,
#                Package_Details.package_header_image
#         FROM Users
#         JOIN Packages ON Users.user_id = Packages.user_id
#         JOIN Package_Details ON Packages.package_id = Package_Details.package_id
#         WHERE Users.user_id = ?
#     ''', (user_id,))
...
#             "package_name": row[4],
#             "package_price": row[5],
#             "package_detail_id": row[6],
#             "package_header_image": row[7],
#             # "package_image": row[8],
#         }
...
#     # If profile_number is 1 or 2, get specific profile types
#     if profile_number in profile_type_map:
#         profile_type = profile_type_map[profile_number]
#         cursor.execute('''
#             SELECT Profile.profile_id, Profile.company_name, Profile.contact_detail, Profile.experience, 
...
#     # If profile_number is 3, get all profiles except "event organizer" and "Venue Provider"
#     elif profile_number == 3:
#         cursor.execute('''
#             SELECT Profile.profile_id, Profile.company_name, Profile.contact_detail, Profile.experience, 
#                    Profile.thumbnail_image, Profile_Type.profile_type, Profile.user_id
...
#     # Check if bidding for this event is already closed
#     if event[8] is not None:
#         conn.close()
#         raise HTTPException(status_code=400, detail="A bid has already been selected for this event")

#     # Check if the bid exists for the event
#     cursor.execute('SELECT * FROM Mark_bidding WHERE bid_id = ? AND event_id = ?', 
#                    (select_bid.bid_id, select_bid.event_id))
#     bid = cursor.fetchone()
...
#     # Update the Event_Organisers table to mark the bid as selected
#     cursor.execute('''
#         UPDATE Event_Organisers
#         SET selected_bid_id = ?
#         WHERE organiser_event_id = ?
#     ''', (select_bid.bid_id, select_bid.event_id))
